...
function renderWizard() {
  ...
  const interiorSelector = document.getElementById("interior-type-selector");
  const ctrlShelves = document.getElementById("ctrl-shelves");
  const ctrlDrawers = document.getElementById("ctrl-drawers");

  if (interiorSelector) interiorSelector.classList.remove("hidden");
  if (ctrlShelves)
    ctrlShelves.classList.toggle(
      "hidden",
      wizardItem.interiorType !== "shelves",
    );
  if (ctrlDrawers)
    ctrlDrawers.classList.toggle(
      "hidden",
      wizardItem.interiorType !== "drawers",
    );
